...
var torrentInfo = function(channel, INSTANCE) {
  ...
  this.selectFile = function (filelist) {
    ...
    channel.say(instance.irc.white.bggreen.bold() + " Would download file: \"" + filelist[0].name.irc.bold() + "\" among " + n + " files. ")
    return null;
  };
}
...
var nthTorrentInfo = function(channel, INSTANCE, n) {
  ...
  this.selectFile = function (filelist) {
    ...
    if( n < 0 || n >= filelist.length) {
      ...
      channel.say(instance.irc.white.bggreen.bold() + " target: \"" + filelist[n].name.irc.bold() + "\" File " + (n+1) + " of " + filelist.length.toString() + ". ")
    }
    return null;
  };
}
...
Torrent.prototype.filename = function(channel, who, message, match) {
  if(this.torrentClient) {
    var name = this.torrentClient.filename();
    if(name) {
      channel.say("TORRENT playing: "+ name);
    }else{
      channel.say("TORRENT still fetching torrent info. Hold on.");
    }
  }else{
