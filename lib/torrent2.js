...
var torrentInfo = function(channel, INSTANCE) {
  ...
  this.selectFile = function (filelist) {
    ...
    channel.say(instance.irc.white.bggreen.bold() + " target: " + filelist[n-1].name + " among " + n + " files. ")
    return null;
  };
}
...
function Torrent(settings) {
  ...
  this.handlers = {
    ...
    " \\? (\\S+)$" : bind(this, this.getTorrentInfo),
...
Torrent.prototype.playFile = function(channel, torrentFilename, selector) {
  ...
  this.torrent-client = new TorrentClient(this.settings,
                            torrentFilename,
                            selector,
                            onTorrentPlay,
                            onTorrentLoaded,
                            onTorrentEnd);
  this.torrent-client.onComplete = function() {
    this.torrent-client = null;
  }
}
