...
TorrentClient.prototype.filename = function() {
  if(this.engine && this.f) {
    var bytes = this.f.length.toString();
    var downloadSpeed = humanFileSize(this.engine.swarm.downloadSpeed(), true) + '/s';
    var unchoked = this.engine.swarm.wires.filter(active);

    var blurb = ' Downloading ' + this.f.name.irc.bold() + '" from ' +
      unchoked.length.toString().irc.bold() + ' peers @' + downloadSpeed.irc.bold();
