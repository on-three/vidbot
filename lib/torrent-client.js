...
function TorrentClient( settings,
  ...
  this.engine.on('ready', function() {
      ...
      if(that.f) {
        ...
        that.mpv = new Mpv(that.settings, '-');
...
TorrentClient.prototype.filename = function(channel, who, message, match) {
  if(this.f) {
    return f.name;
  }
  // if(this.peerflix) {
...
TorrentClient.prototype.pause = function(channel, who, message, match) {
  var on = true;
  if(this.mpv) {
    this.mpv.pause(on);
  }else{
...
TorrentClient.prototype.unpause = function(channel, who, message, match) {
  var on = false;
  if(this.mpv) {
    this.mpv.pause(on);
  }else{
...
TorrentClient.prototype.mute = function(channel, who, message, match) {
  var on = true;
  if(this.mpv) {
    this.mpv.mute(on);
  }else{
...
TorrentClient.prototype.unmute = function(channel, who, message, match) {
  var on = false;
  if(this.mpv) {
    this.mpv.mute(on);
  }else{
...
TorrentClient.prototype.subs = function(channel, who, message, match) {
  var arg = match[1];
  var on = true;
  if(arg=='off'){on=false;}
  if(this.mpv) {
...
TorrentClient.prototype.subtitleTrack = function(channel, who, message, match) {
  var id = match[1];
  if(this.mpv) {
...
TorrentClient.prototype.audioTrack = function(channel, who, message, match) {
  var id = match[1];
  if(this.mpv) {
...
TorrentClient.prototype.seekSeconds = function(channel, who, message, match) {
  var s = match[1];
  if(this.mpv) {
