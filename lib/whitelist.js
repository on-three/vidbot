...
var isWhitelisted = function(who) {
  console.log("isWhitelisted invoked on person " + who);
  if(modlistIsOn) {
    console.log("modlist on so checking modlist");
    return isModlisted(who);
  }else if(whitelistIsOn) {
    console.log("whitelist on so checking whitelist.");
...
var whitelisted = function(thisValue, f) {
  ...
  return function(channel, who, message, match) {
    console.error("whitelisted invoked.");
    if(isWhitelisted(who)) {
      return function() {
        return f.apply(thisValue, [channel, who, message, match])
      }
    }else{
      return message;
    }
...
var modlisted = function(thisValue, f) {
  ...
  // channel, who, message, match
  return function(channel, who, message, match) {
    console.error("modlisted invoked.");
    if(isModlisted(who)) {
      return function() {
        return f.apply(thisValue, [channel, who, message, match])
      }
