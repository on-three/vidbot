...
function Shows(settings) {
  ...
  // show lists.
  for(var show in this.showList) {
    //console.log("A SHOW : ", show);
    for(var subgroup in this.showList[show]) {
      // if(!show.hasOwnProperty(subgroup)) {
      //   continue;
      // }
      //console.log("A SUBGROUP: ", subgroup);
      if(this.showList[show][subgroup].hasOwnProperty('regexes')) {
        for(var i = 0; i< this.showList[show][subgroup].regexes.length; i++) {
          // Run each regex in the filesystem, appending all results to this showlist.
          if(!this.showList[show][subgroup].hasOwnProperty('episodes')) {
            this.showList[show][subgroup].episodes = [];
          }

          var getFiles = function(thisValue, show, subgroup) {
            //console.log("REGEX: ", this.showList[show][subgroup].regexes[i]);
            glob(thisValue.showList[show][subgroup].regexes[i], null, function (er, files) {
              for(var f = 0; f < files.length; f++) {
                //console.log("APPENDING file: ", files[f]," to: ", show," ",subgroup);

                that.showList[show][subgroup]["episodes"].push(files[f]);
              }
            });
          };
          getFiles(this, show, subgroup);
        }
      }
    }
  }
  ...
  this.handlers = {
    ...
    //" list( \\s+)?( \\s+)?$" : whitelisted(this, this.list),
    //" set (\\s+) ((e|ep|episode)?\\d+)?$" : whitelisted(this, this.setShow),
  };
}
...
Shows.prototype.getNextEpisode = function() {
  ...
  while(nextEpisode == null)
  {
    ...
    console.log('current subgroup index; ', this.showsData[nextShowName].currentSubgroupIndex);
    var nextSubGroupName = subgroups[this.showsData[nextShowName].currentSubgroupIndex];
    console.log("current subgroup name: ", nextSubGroupName);
    ...
    var numEpisodes = this.showList[nextShowName][nextSubGroupName].episodes.length;
    console.log("Num episodes for ", nextShowName, " sg: ", nextSubGroupName, " is ", numEpisodes);
    if(this.showsData[nextShowName].currentEpisode >= numEpisodes) {
      this.showsData[nextShowName].currentEpisode = 0;
      // also increment the subgroup ('season')
      this.showsData[nextShowName].currentSubgroupIndex++;
      if(this.showsData[nextShowName].currentSubgroupIndex >= subgroups.length) {
        this.showsData[nextShowName].currentSubgroupIndex = 0;
      }
    }
    nextEpisode = this.showList[nextShowName][nextSubGroupName].episodes[this.showsData[nextShowName].currentEpisode];
  }
...
// Shows.prototype.list = function(channel, who, message, match) {
//   var showNames = Object.keys(this.showList);
//   if(match.length==2) {
//     // we have an possible show name to list seasons for
//   } else if(match.length==3) {
//     // show number of episodes
//
//   } else {
//     channel.say(showNames.join(' '));
//   }
//   return null;
// }
//
// Shows.prototype.setShow = function(channel, who, message, match) {
//   var s = match[1];
//   if(this.mpv) {
//     this.mpv.seekSeconds(channel, s);
//   }
