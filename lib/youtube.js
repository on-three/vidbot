...
var QUIT_REGEX = /^\.\.y (q|quit|wipe|next)$/i;
var WIPE_ALL_REGEX = /^\.\.y wipe all$/i;
var PLAY_REGEX = /^\.\.y (\S+)$/i;
var ON_REGEX = /^\.\.y on$/i;
var OFF_REGEX = /^\.\.y off$/i;
var SUBS_VISIBLE_REGEX = /^\.\.y subs (on|off)$/i;
var SUBS_TRACK_REGEX = /^\.\.y subs (\d)$/i;
var AUDIO_TRACK_REGEX = /^\.\.y audio (\d)$/i;
...
exports.handleMessage = function(channel, who, message) {
  ...
  if(message.match(Y_REGEX)){
    ...
    pause(channel, match[1])
    return null;
  }else if(message.match(QUIT_REGEX)) {
    quit(channel)
    return null;
  }else if(message.match(WIPE_ALL_REGEX)) {
    wipeAll(channel)
    return null;
  }else if(message.match(MUTE_REGEX)) {
    var match = MUTE_REGEX.exec(message);
    mute(channel, match[1])
    return null;
  }else if(message.match(SUBS_VISIBLE_REGEX)) {
    var match = SUBS_VISIBLE_REGEX.exec(message);
    subs(channel, match[1])
    return null;
  }else if(message.match(SUBS_TRACK_REGEX)) {
    var match = SUBS_TRACK_REGEX.exec(message);
    subtitleTrack(channel, match[1])
    return null;
  }else if(message.match(AUDIO_TRACK_REGEX)) {
    var match = AUDIO_TRACK_REGEX.exec(message);
    audioTrack(channel, match[1])
    return null;
...
function subtitleTrack(channel, id) {
  if(mpv) {
    mpv.subtitleTrack(channel, id);
  }
}

function audioTrack(channel, id) {
  if(mpv) {
