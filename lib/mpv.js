...
function Mpv(settings, file, ch) {
  ...
  this.proc = spawn('mpv', [this.file, '--input-unix-socket=/tmp/mpvsocket_'+this.INSTANCE, '--mute=no'], { stdio: 'inherit', env: env });
...
Mpv.prototype.audioTrack = function(id) {
  var that = this;
  var onSuccess = function(response) {
    //that.channel.say(that.name()+' pause '+ that.on.toString().irc.bold());
  }
  ...
  this.mpv_send('{ "command": ["set_property", "aid", "'+ id.toString() +'""] }\n',
    onSuccess,
    onFailure);
}

Mpv.prototype.subtitleTrack = function(id) {
  var that = this;
  var onSuccess = function(response) {
    //that.channel.say(that.name()+' pause '+ that.on.toString().irc.bold());
  }
  ...
  this.mpv_send('{ "command": ["set_property", "sid", "'+ id.toString() +'"] }\n',
    onSuccess,
    onFailure);
}
